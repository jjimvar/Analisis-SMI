...
*Estimamos el modelo
logit afecta hombre $edad $paisnac $educ c.relab i.comunidad $x2, difficult iterate(10)

keep if train==0 //Nos movemos en la base de datos de prueba

*Generamos sens spec
lsens, genprob(cutoff) gensens(sens) genspec(spec)

*Generamos variable a maximizar: media sens y spec
gen clasmed=(sens+spec)/2
egen opt=pctile(clasmed), p(99.99)
gen cutopt=cutoff if clasmed>=opt
qui sum cutopt
