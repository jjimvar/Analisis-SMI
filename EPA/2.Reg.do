...
***************************Modelo BIC PROV***************************************
********************************************************************************
*Creamos variable afectado por la reform
gen tratado=.
replace tratado=1 if ybicprov>=0.150863595
replace tratado=0 if ybicprov<0.130863595 & ybicprov>p5bicprov
**Afectado por año
gen did=.
...
est store modelo11
**Con Xi
qui reg activo $a $b if empleado!=. & tratado!=., robust
estadd local FE "(Prov)"
estadd local Xi "(Si)"
qui sum activo
estadd scalar Promedio=r(mean)*100
...
********************************************************************************
*Actividad
esttab modelo11 modelo12 modelo21 modelo22 using 2.RegAct.rtf, keep(anoenc tratado did) ///
	title("Regresión 1: Participación mercado laboral") ///
...
*Empleo
esttab modelo13 modelo14 modelo23 modelo24 using 2.RegEmpl.rtf, keep(anoenc tratado did) ///
	title("Regresión 2: Participación empleo") ///
	mtitles("Lasso BIC" "Lasso BIC" "Lasso CV" "Lasso CV") ///
	s(FE Xi Promedio R2 Nobs) ///
	coeflabels(anoenc "Diferencia" tratado "Tratado" did "DiD") ///
	star(* 0.1 ** 0.05 *** 0.01) replace